...
namespace FM
{
	...
	public:
		...
		int32_t		dbgGetOpOut(int c, int s) { return ch[c].op[s].dbgopout_; }
		int32_t		dbgGetPGOut(int c, int s) { return ch[c].op[s].dbgpgout_; }
...
//		void	SetChannelMask(uint32_t mask);
	...
	// リズム音源関係
		...
		int32_t	mode288;		// 288/2608モードフラグ
	};
}
