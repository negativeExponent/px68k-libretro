...
static const uint8_t DAT_TABLE[32] = {
...
static int32_t FASTCALL FDC_Int(int32_t irq)
{
		...
		if ((FDD_IsReady(fdc.drv)) || (fdc.ready))
		{
			...
			else
			{
				...
				else
				{
					...
					if (memcmp(&id, &retid, sizeof(FDCID)))
						fdc.st1 |= 0x24;
					if (ret & 2)
						fdc.st2 |= 0x40;
					if (ret & 0x0c)
		...
		if ((FDD_IsReady(fdc.drv)) || (fdc.ready))
		{
			...
			else
			{
				...
				else
				{
					...
					if ((fdc.cmd != 6) && (fdc.cmd != 12))
						fdc.wexec = 1;
					if (((ret & 2) && (fdc.cmd != 12)) || ((!(ret & 2)) && (fdc.cmd == 12)))
						fdc.st2 |= 0x40;
					if (ret & 0x1c)
						fdc.st0 |= 0x40;
					if (ret & 0x0c)
...
void FASTCALL FDC_Write(uint32_t adr, uint8_t data)
{
	if (adr == 0xe94003)
	{
		...
		else
		{
			...
			if (!fdc.wrnum)
			{
				...
				fdc.st1   = 0;
				fdc.st2   = 0;
				if ((fdc.cmd == 17) || (fdc.cmd == 25) || (fdc.cmd == 29))
					fdc.st2 |= 8;
				FDC_ExecCmd();
			}
		}
	...
	{
		if ((fdc.ctrl & 0x01) && (!(data & 0x01)))
		{
			/* Drive0 control (Down edge) */
			if (fdc.ctrl & 0x20)
				FDD_EjectFD(0);
			FDD_SetEMask(0, (fdc.ctrl & 0x40) ? 1 : 0);
			FDD_SetBlink(0, (fdc.ctrl & 0x80) ? 1 : 0);
		}
		if ((fdc.ctrl & 0x02) && (!(data & 0x02)))
		{
			/* Drive1 control (Down edge) */
			if (fdc.ctrl & 0x20)
				FDD_EjectFD(1);
